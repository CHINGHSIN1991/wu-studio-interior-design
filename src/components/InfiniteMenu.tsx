...
void main() {
    ...
    float cellAspect = 1.0; // 每個格子是正方形
    
    // 中心點為(0.5, 0.5)
    vec2 centeredUV = vUvs - 0.5;
    
    // 使用 "cover" 策略，但保持圖片原始寬高比
    float scaleFactor;
    if (textureAspect > cellAspect) {
        // 圖片較寬，按高度縮放
        scaleFactor = cellAspect / textureAspect;
        centeredUV.x /= scaleFactor;
    } else {
        // 圖片較高，按寬度縮放
        scaleFactor = textureAspect / cellAspect;
        centeredUV.y /= scaleFactor;
    }
    
    // 將UV值移回[0,1]範圍
    vec2 finalUV = centeredUV + 0.5;
    
    // 垂直翻轉UV坐標以匹配圖片方向
    finalUV.y = 1.0 - finalUV.y;
    
    // 限制UV坐標在[0,1]範圍內，避免重複
    finalUV = clamp(finalUV, 0.0, 1.0);
    
    // 映射到正確的圖集單元格
    finalUV = finalUV * cellSize + cellOffset;
    
    outColor = texture(uTex, finalUV);
    outColor.a *= vAlpha;
}
`
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {activeItem && (
          ...
          <a
            href={
              activeItem.link?.startsWith('http')
                ? activeItem.link
                : `projects/${activeItem.link}`
            }
            target={activeItem.link?.startsWith('http') ? '_blank' : ''}
            rel={
              activeItem.link?.startsWith('http') ? 'noopener noreferrer' : ''
            }
            className={`
          absolute
          left-1/2
        ...
        `}
          >
            <span className="select-none relative text-[#060606] top-[2px] text-[26px]">
              &#x2197;
            </span>
          </a>
        </>
